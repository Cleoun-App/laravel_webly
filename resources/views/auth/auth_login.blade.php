                        ...
                        <div class="card card-plain">
                            ...
                            <div class="card-body">
                                <form role="form" action="{{ route('login') }}" method="POST">
                                    @csrf
                                    <div class="mb-3">
                                        <input type="text" class="form-control form-control-lg" name="email" placeholder="Email"  value="admin@mail.io" aria-label="Email">
                                        @error('email')
                                            <span style="color:#ff3131; font-size: 13px;">{{$message}}</span>
                                        @enderror
                                    </div>
                                    <div class="mb-3">
                                        <input type="password" class="form-control form-control-lg" name="password" placeholder="Password" aria-label="Password" value="password">
                                        @error('password')
                                            <span style="color:#ff3131; font-size: 13px;">{{$message}}</span>
                                        @enderror
                                    </div>
